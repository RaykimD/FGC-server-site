  ...
  return (
    <div className="animate-fade-in pb-10 max-w-3xl mx-auto mt-4 select-none">
      <div className="mb-8 text-center flex flex-col items-center">
        ...
        <p className="text-sm font-medium text-slate-500 dark:text-gray-400 mt-2">인게임 스탯 제보와 웹사이트 오류를 제보해주세요!</p>
      </div>
      ...
      {!showForm ? (
        <div className="animate-fade-in">
          <div className="flex justify-between items-end mb-4 px-2">
            ...
            <button onClick={() => { setShowForm(true); setActiveTab('info'); }} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded-xl shadow-md transition-all">✍️ 제보하기</button>
          </div>
           ...
           <div className="flex justify-between items-start mb-6">
              <div className="flex gap-4">
                <button 
                  ...
                  className={`text-lg font-black pb-2 px-1 transition-all border-b-2 ${activeTab === 'info' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
                >
                  📊 인게임 스탯 갱신
                </button>
                <button 
                  onClick={() => setActiveTab('bug')} 
                  className={`text-lg font-black pb-2 px-1 transition-all border-b-2 ${activeTab === 'bug' ? 'border-red-500 text-red-600 dark:text-red-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
                >
                  🐛 사이트 버그/건의
                </button>
              </div>
