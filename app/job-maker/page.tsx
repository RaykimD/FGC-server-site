...
export default function JobMakerPage() {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      // 💡 구형 html2canvas 대신 모던 CSS(lab 컬러 등)를 완벽 지원하는 html-to-image 사용
      ...
      // 💡 캔버스가 아닌 dataUrl 형식으로 바로 뽑아냅니다.
      const dataUrl = await (window as any).htmlToImage.toPng(element, {
        backgroundColor: '#1e293b', // 빈 공간을 채워줄 짙은 남색 배경
        pixelRatio: 2, // 화질을 2배로 선명하게 캡처
      });
  ...
  // 💡 아프리카TV 이미지를 우회 API로 연결해주는 함수
  const getProxiedImageUrl = (streamerId: string) => {
    const firstTwo = streamerId.substring(0, 2).toLowerCase();
    const rawUrl = `https://profile.img.afreecatv.com/LOGO/${firstTwo}/${streamerId}/${streamerId}.jpg`;
              ...
              >
                {(assignments[pair.base.id] || []).map(streamer => (
                  <div key={streamer.id} draggable onDragStart={(e) => onDragStart(e, streamer, pair.base.id)} onClick={() => removeFromJob(pair.base.id, streamer.id)} className="relative group cursor-grab active:cursor-grabbing flex flex-col items-center w-16 hover:-translate-y-1 transition-transform">
                    {/* 💡 프록시 URL 사용 (crossOrigin 제거) */}
                    <img src={getProxiedImageUrl(streamer.id)} className={`w-14 h-14 rounded-full object-cover border-2 ${pair.base.ringColor} shadow-md bg-slate-800`} />
              ...
              >
                {(assignments[pair.hidden.id] || []).map(streamer => (
                  <div key={streamer.id} draggable onDragStart={(e) => onDragStart(e, streamer, pair.hidden.id)} onClick={() => removeFromJob(pair.hidden.id, streamer.id)} className="relative group cursor-grab active:cursor-grabbing flex flex-col items-center w-20 hover:-translate-y-1 transition-transform">
                    <img src={getProxiedImageUrl(streamer.id)} className={`w-16 h-16 rounded-full object-cover border-[3px] ${pair.hidden.ringColor} bg-slate-800`} />
        ...
        <div className="p-3 flex-1 overflow-y-auto custom-scrollbar flex flex-wrap content-start gap-2 bg-slate-900/40">
          {getUnassignedMembers().length === 0 ? (
            ...
            getUnassignedMembers().map(streamer => (
              ...
              >
                <img src={getProxiedImageUrl(streamer.id)} className="w-10 h-10 rounded-full object-cover pointer-events-none border border-slate-500 bg-slate-800" />
