...
const GuildLogo = ({ guildName }: { guildName: string }) => {
  const [error, setError] = useState(false);
  // 띄어쓰기를 없애서 파일명을 안전하게 찾습니다 (예: "오아 길드" -> "오아길드.png")
  const safeName = guildName.replace(/\s+/g, '');

  if (error) {
    return (
      <div className="w-16 h-14 rounded-2xl bg-slate-100 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 flex items-center justify-center text-3xl group-hover:scale-110 transition-transform overflow-hidden shrink-0">
        🛡️
      </div>
    );
  }
  ...
  return (
    <div className="w-16 h-14 rounded-2xl bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 flex items-center justify-center group-hover:scale-110 transition-transform overflow-hidden shrink-0 shadow-inner">
      <img 
                ...
                <Link href={`/status/${guild.id}`} key={guild.id} className="group block h-full">
                  <div className="bg-[#f8fafc] dark:bg-[#1a1a1a] rounded-3xl p-8 border border-slate-200 dark:border-gray-800 shadow-md hover:shadow-xl hover:border-blue-500 dark:hover:border-blue-500 transition-all h-full flex flex-col">
                    ...
                    <div className="flex items-center gap-5 mb-6 border-b border-slate-200 dark:border-gray-800 pb-5">
                      <GuildLogo guildName={guild.name} />
                      <div>
